...
def Preprocess(image): # returns perspective transformed binary image of sudoku square in a 450*450 window
    ...
    for cnt in contours:
        ...
        if area > 4000:                                             # To detect the biggest square 
            ...
            cap= cv2.drawContours(img, [approx], 0, (255, 0, 0), 5)      # Drawing blue countour over approximated square
...
# Testing the code
img = cv2.imread('C://Users//Pallavi//Pictures//sudoku.JPG')
res = Preprocess(img)
...
finalres = cv2.warpAffine(res, M, (h, w))

cv2.imshow('img',img)
cv2.imshow('ans',finalres)
cv2.waitKey(0)
